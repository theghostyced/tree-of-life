...
<TooltipPrimitive.Root variant="none">
	<div
		bind:this={ref}
		class={cn(
			"border-line-strong bg-surface text-ink grid min-w-[9rem] items-start gap-1.5 rounded-lg border px-2.5 py-1.5 text-xs shadow-card",
			className
		)}
		{...restProps}
	>
		...
		<div class="grid gap-1.5">
			{#each visibleSeries as item, i (item.key + i)}
				...
				<div
					class={cn(
						"[&>svg]:text-muted flex w-full flex-wrap items-stretch gap-2 [&>svg]:size-2.5",
